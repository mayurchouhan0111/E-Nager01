  ...
  try {
    ...
    if (!isOfficer) {
      ...
      if (!activeEmail && !activeUid && !derivedMobile && !activeName) {
        return localItems.filter(item => !(item.userEmail || item.userUid || item.userMobile || item.applicantDetails?.email || item.applicantDetails?.mobile || item.informantDetails?.email || item.informantMobile));
      }
