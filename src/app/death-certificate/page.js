...
export default function DeathCertificatePage() {
  ...
  const loadApplications = async (overrideEmail = null, isBackground = false) => {
    ...
    try {
      ...
      console.warn('Error loading death applications:', e);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans antialiased">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn btn-primary w-full sm:w-auto flex items-center justify-center gap-2 shadow-lg shadow-emerald-700/20 font-bold"
              >
                {loading ? (
        ...
        {selectedApp && !showCertModal && !showLetterModal && !showOfficialDocModal && (
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[9999] overflow-y-auto p-2 sm:p-4 flex items-center justify-center pt-2 sm:pt-6">
            <div className="bg-white border border-slate-200 rounded-3xl max-w-3xl w-full p-4 sm:p-6 shadow-2xl space-y-4 max-h-[92vh] flex flex-col relative overflow-hidden">
              <div className="flex items-start justify-between border-b border-slate-100 pb-3 shrink-0">
                <div>
                  <span className="text-[10px] sm:text-xs font-mono font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded border border-emerald-200">
                    {selectedApp.applicationNo || 'DRAFT'}
                  </span>
                  <h3 className="text-slate-900 font-extrabold text-sm sm:text-lg mt-1">
                    मृतक: स्व. {selectedApp.deceasedDetails?.fullName}
                  </h3>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <button 
                    onClick={() => loadApplications()} 
                    className="px-2.5 py-1 rounded-xl bg-emerald-50 text-emerald-800 hover:bg-emerald-100 text-[11px] sm:text-xs font-bold border border-emerald-200 flex items-center gap-1 transition-all"
                    title="लाइव सर्वर स्थिति रीफ्रेश करें"
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />
                    <span className="hidden sm:inline">ताज़ा करें</span>
                  </button>
                  <button onClick={() => setSelectedApp(null)} className="p-1.5 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>

              <div className="overflow-y-auto flex-1 pr-1 space-y-4 pt-1 pb-6">
                {selectedApp.lastOfficerRemark && (
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 sm:p-4 text-xs text-amber-900 space-y-1">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 sm:p-4 text-xs space-y-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="btn btn-secondary btn-sm text-xs font-bold text-emerald-900 bg-emerald-50 hover:bg-emerald-100 flex items-center gap-1.5 py-2 px-3 rounded-xl border border-emerald-200 w-full sm:w-auto justify-center"
                  >
                    <Printer className="w-4 h-4 text-emerald-700" /> भौतिक पावती पत्र देखें / प्रिंट करें
                  </button>
                </div>

                <h4 className="text-xs uppercase font-extrabold text-slate-500 tracking-wider pt-2">आवेदन टाइमलाइन विवरण (Activity Timeline)</h4>
