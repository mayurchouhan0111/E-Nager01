...
export default function BirthCertificatePage() {
  ...
  useEffect(() => {
    ...
    let unsubFirestore = () => {};
    try {
      unsubFirestore = onSnapshot(collection(db, 'birthCertificates'), () => {
        triggerSilentRefresh();
      }, err => console.warn('[BirthCert] Live listener note:', err));
    } catch (e) {}
  ...
  const loadApplications = async (overrideEmail = null, isBackground = false) => {
    ...
    try {
      ...
      console.warn('Error loading birth applications:', e);
    } finally {
          ...
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[9999] overflow-y-auto p-2 sm:p-4 flex items-center justify-center pt-2 sm:pt-6">
            <div className="bg-white border border-slate-200 rounded-3xl max-w-3xl w-full p-4 sm:p-6 shadow-2xl space-y-4 max-h-[92vh] flex flex-col relative overflow-hidden">
              <div className="flex items-start justify-between border-b border-slate-100 pb-3 shrink-0">
                <div>
                  <span className="text-[10px] sm:text-xs font-mono font-bold text-blue-800 bg-blue-100 px-2 py-0.5 rounded border border-blue-200">
                    {selectedApp.applicationNo || 'DRAFT'}
                  </span>
                  <h3 className="text-slate-900 font-extrabold text-sm sm:text-lg mt-1">
                    शिशु: {selectedApp.childDetails?.fullName || 'अनाम'}
                  </h3>
                </div>
              ...
              <div className="overflow-y-auto flex-1 pr-1 space-y-4 pt-1 pb-6">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="btn btn-secondary btn-sm text-xs font-bold text-blue-900 bg-blue-50 hover:bg-blue-100 flex items-center gap-1.5 py-2 px-3 rounded-xl border border-blue-200 w-full sm:w-auto justify-center"
                  >
                    <Printer className="w-4 h-4 text-blue-700" /> भौतिक पावती पत्र देखें / प्रिंट करें
                  </button>
                </div>
